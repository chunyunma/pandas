...
def mypandasbp(data):
	...
	# create a copy of data; filter out practice trials
	databp = data[data.Block_Name.str.contains('block')]
	# select trials with correct and timely response
	databp = databp[databp.TF & (databp.Marked == False)]
	

	for condition, df in databp.groupby(['Operation', 'SOA']):
		bp = plt.boxplot(df.RT.values, whis = 3)    # only identify outliers outside of 3IQR, 
	#emulating what SPSS does. The tricky part is "RT.values". Normally, df.RT should work 
	#with boxplot. But in this case, it kept raising KeyError. Series.values return series as an ndarray
...
# 			pdb.set_trace()
			bottom_points = sorted(bottom_points, reverse=True)    # use sorted() instead of 
# 			sort to accommodate lists with only one element
...
# 			pdb.set_trace()
			...
			databp.update(df)	
# 	pdb.set_trace()
	data.update(databp)    # update column "outliers" in the original dataframe--data"
	return data

# export to csv if using this module along
# data.to_csv('playground/data.txt', sep = '\t', index = False)

# pandas also has data.boxplot(). But it does not return a dictionary like plt.boxplot(df). 
